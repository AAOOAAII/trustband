...
class UngatedBaseline:
    ...
    def govern_issue(self, sess: int, tier: int, nonce: int, epoch: int = 0) -> Cap:
        ...
        return Cap(sess=sess, tier=tier, nonce=nonce, epoch=epoch,
                   policy=b"", sess_epoch=0, tag=b"whatever")
...
def r2():
    u = fresh_ungated()
    forged_u = Cap(SESS, 2, 99, 0, b"", 0, b"\xde\xad\xbe\xef" * 8)
    ...
    forged = Cap(SESS, 2, 99, g.gov_epoch, g.gov_policy, 0, b"\xde\xad\xbe\xef" * 8)
    ...
    p2 = g2.ingest(Channel.SESSION_DEMUX,
                   Cap(SESS, 2, 99, g2.gov_epoch, g2.gov_policy, 0, b"\x00" * 32))
...
def r3():
    ...
    forged_now = Cap(SESS, 2, 4242, gl.gov_epoch, gl.gov_policy, 0,
                     compute_tag(leaked, SESS, 2, 4242, gl.gov_policy, 0))
    ...
    still_old = Cap(SESS, 2, 4243, 0, gl.gov_policy, 0,
                    compute_tag(leaked, SESS, 2, 4243, gl.gov_policy, 0))
...
def main() -> int:
    ...
    _watch = [p for p in ("trustband", "scripts/warrantable_battery.py")
              if (REPO / p).exists()]
    if len(_watch) < 2:
